...
import { patchState, getState, setProjectContext } from "./state/projectState.js";
import { emit, EVENTS, on } from "./services/eventBus.js";
import { emitFormsChanged, bindFormsChanged } from "./services/syncService.js";
...
const viewer = new DocumentViewer({
  ...
  onAddPlanRow: () => {
    window.location.hash = "#batchPlan";
    activateTabFromHash();
    planEditor.addRowFromSelectedTemplate(templateTree.getTemplateOptions().find((item) => item.templateNodeId === getState().selectedTemplateNode?.id) || null);
  },
...
const planEditor = new PlanEditor({
  ...
  getSelectedTemplateOption: () => {
    const selected = getState().selectedTemplateNode;
    return templateTree.getTemplateOptions().find((item) => item.templateNodeId === selected?.id) || null;
  },
  getTemplateOptions: () => templateTree.getTemplateOptions(),
  getUnitProjectId: () => getState().activeUnitProjectId,
  onRefresh: async () => loadInspectionPlans(),
  onDeletePlan: async (planId) => {
    const { project, activeUnitProjectId } = getState();
    const result = await deleteInspectionPlan(project.projectId, activeUnitProjectId, planId);
    showResult("#batchPlanResult", result);
    await loadInspectionPlans();
  },
  onSavePlan: async (planId, payload) => {
    const { project } = getState();
    const result = String(planId || "").startsWith("draft-plan:")
      ? await createInspectionPlan(project.projectId, payload)
      : await updateInspectionPlan(project.projectId, planId, payload);
    showResult("#batchPlanResult", result);
    return result;
  },
  onPreviewPlan: async (planId) => {
    const { project } = getState();
    const result = await previewInspectionPlan(project.projectId, planId);
    showResult("#batchPlanResult", result);
    return result;
  },
  onGeneratePlan: async (planId, payload) => {
    const { project, activeUnitProjectId } = getState();
    const result = await generateInspectionPlan(project.projectId, planId, {
      ...payload,
  ...
  onOpenDocument: async (documentId) => {
    const { project, activeUnitProjectId } = getState();
    const result = await openDocument(project.projectId, activeUnitProjectId, documentId);
    showResult("#batchPlanResult", result);
  },
  onLoadCapacityConfigs: async (templateNodeId) => {
    const { project, activeUnitProjectId } = getState();
    return getCapacityConfigs(project.projectId, activeUnitProjectId, templateNodeId);
  }
});
...
async function loadInspectionPlans() {
  ...
  const result = await listInspectionPlans(project.projectId, activeUnitProjectId);
  patchState({
    inspectionPlans: result.plans || [],
    currentInspectionPlan: (result.plans || [])[0] || null
  });
  planEditor.setData(result);
}
...
function bindUiEvents() {
  ...
  $("#refreshStatus")?.addEventListener("click", () => boot());
  $("#retryServiceStatus")?.addEventListener("click", () => boot());
...
async function updateServiceStatus() {
  ...
  try {
    ...
    statusElement.textContent = `服务正常｜服务 ${health.version || "-"}｜AI ${health.aiEnabled ? "已启用" : "未启用"}`;
...
boot().catch((error) => {
  ...
  showResult("#batchPlanResult", error);
